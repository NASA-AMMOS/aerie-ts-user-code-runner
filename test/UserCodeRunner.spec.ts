...
describe('behavior', () => {
	describe('SECURITY', () => {
      ...
      `.trimTemplate();
			...
			const result = await runner.executeUserCode(userCode, [], 'string', []);
      ...
      `.trimTemplate();
			...
			const result = await runner.executeUserCode(userCode, [], 'void', []);
    ...
    `.trimTemplate();
		...
		const result = await runner.executeUserCode(userCode, ['hello'], 'string', ['string']);
    ...
    `.trimTemplate();
		...
		const result = await runner.executeUserCode(userCode, ['hello'], 'number', ['string']);
    ...
    `.trimTemplate();
		...
		try {
			await runner.executeUserCode(userCode, ['hello'], 'string', ['string'], 1000, [
				ts.createSourceFile(
					'additionalFile.ts',
					`
      export {}
    ...
    `.trimTemplate();
		...
		const result = await runner.preProcess(userCode, 'string', ['string']);
...
describe('regression tests', () => {
  ...
  `.trimTemplate();
    ...
    `.trimTemplate();
		...
		expect(result.unwrapErr().length).toBe(1);
		expect(result.unwrapErr()[0].message).toBe(
			`TypeError: TS2322 Incorrect return type. Expected: 'Goal | Promise<Goal>', Actual: 'FakeGoal'.`,
		);
		expect(result.unwrapErr()[0].stack).toBe(
			`
    at (5:1)
      ...
      `.trimTemplate();
		...
		const [constraintsAst, constraintsEdsl, modelSpecific] = await Promise.all([
			...
			fs.promises.readFile(
				new URL('./inputs/missing-location/constraints-edsl-fluent-api.ts', import.meta.url).pathname,
				'utf8',
			),
			fs.promises.readFile(
				new URL('./inputs/missing-location/mission-model-generated-code.ts', import.meta.url).pathname,
				'utf8',
			),
		]);

		const result = await runner.executeUserCode(userCode, [], 'Constraint', [], undefined, [
